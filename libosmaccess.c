 ...
 */
void filter_cp_op(filterop* to, filterop* from) {
  to->type = from->type;
  if(from->type == CondKV) {
    to->kv_cond.key = strdup(from->kv_cond.key);
    to->kv_cond.val = strdup(from->kv_cond.val);
  } else if(from->type == CondKVre) {
    to->kv_re_cond.key = strdup(from->kv_re_cond.key);
    to->kv_re_cond.val = strdup(from->kv_re_cond.val);
  } else if(from->type == CondType) 
    to->type_cond = from->type_cond;
}


/** Create a new filter defined by a sequence of filteroperations.
 */
filter* newFilterInit(filterop* fa) {
  filter* f = malloc(sizeof(filter));
  int maxStackSize = 0; // count stack
